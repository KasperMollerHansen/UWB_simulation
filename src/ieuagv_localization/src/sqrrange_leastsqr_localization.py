...
def position_calculation(sensor_pos,dist): 
    flag_1 = False
    try:
        vertical=len(sensor_pos[0]) #number of dimensions in anchor position data
    except TypeError:
    ...
    if (flag_1 == True):
        ...
        horizontal=len(sensor_pos) #number of anchors available
        A=(-2*sensor_pos).transpose() #transposed sensor_pos multiplied by -2
        B=np.ones([horizontal,1],dtype=float).transpose()
        A=np.append(A,B,axis=0)
        A=A.transpose() # here we have a 3x4 matrix with columns corresponding to [x,y,z,1]
